(function (window, undefined) {
    ...
    var document = window.document,
	    ...
	    defaultHeight = 100,
	    menus,  //存储menuconfiguration
		currentRange,      //记录当前选中范围
        $txt = $('<div contenteditable="true" class="textarea" ></div>'),  //编辑区
        $btnContainer = $('<div class="btn-container"></div>'), //menu容器
    ...
    //get可以插入 table格的元素，用于 commandHooks['insertHTML']
    ...
    function updateMenuStyle() {
        ...
        $allMenusWithCommandName.each(function(){
            ...
            if(commandName === 'insertunorderedlist' || commandName === 'insertorderedlist'){
                return;  //ff中，如果is刚刷新的页面，no选中文本的情况下，执行这两个的 queryCommandState 报 bug
            }
    ...
    menus = [
        {
            ...
            'dropMenu': (function(){
                var arr = [],
                    //注意，此处commandValue必填项，no则程序不会跟踪
            ...
            'modal': (function(){
                var rowNumTxtId = getUniqeId(),
                    ...
                    $modal = $(
                        ...
                        '    view题行：<input id="' + titleCheckId + '" type="checkbox">' +
                        '   <button id="' + btnId + '">插入 table格</button>',
                        '</div>'
                    ),
                    callback = function(){
    ...
    $.fn.wangEditor = function(customMenus){
        ...
        function createMenuElem(menu){
            ...
            var type = menu.type,
                ...
                $btn = $('<a class="btn btn-default" href="#"></a>');  //一定要有 herf='#'，no则no法监听blur事件
            ...
            if(type === 'btn'){
                ...
                if(typeof command === 'function'){
                    $btn.click(function(e){
                        command(e);  //如果commandis函数，则直接执行command
                    });
                }
            }
            //下拉menu
            ...
            if(title){
                ...
                $btn.mouseenter(function(){
                    ...
                    timer = setTimeout(showToolTip, 200);  //0.2s之后才 viewtooltip，防止鼠快速经过时会闪烁
                }).mouseleave(function(){
                    clearTimeout(timer);
                    $toolTip.hide();
                });
    	...
    	//$txt光发生变化时，saveselection，更新menu style
