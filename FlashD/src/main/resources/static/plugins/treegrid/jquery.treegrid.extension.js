(function($) {
    ...
    $.fn.bootstrapTreeTable = function(options, param) {
        ...
        target.getRootNodes = function(data) {
            ...
            $.each(data, function(index, item) {
                ...
                // add一个默认attribute，用来判断当前节点有没有被 view
                item.isShow = false;
            });
            return result;
        };
        ...
        target.load = function(parms){
            ...
            target.append(tbody);
            // add加载loading
            var _loading = '<tr><td colspan="'+options.columns.length+'"><div style="display: block;text-align: center;">正在努力地加载data中，请稍候……</div></td></tr>'
            tbody.html(_loading);
            // 默认高度
            if(options.height){
            ...
            $.ajax({
                ...
                success : function(data, textStatus, jqXHR) {
                    ...
                    // 下边的operate主要is为了query时让一些没有根节点的节点 view
                    $.each(data, function(i, item) {
                        if(!item.isShow){
                            var tr = $('<tr></tr>');
                            tr.addClass('treegrid-' + (j + "_" + i));
                    ...
                    // 初始化treegrid
                    target.treegrid({
                        treeColumn: options.expandColumn?options.expandColumn:(hasSelectItem?1:0),//如果有radio或checkbox默认第二列层级 view，当前is在 user未设置的提前下
        ...
        if (options.url) {
            ...
            // 也可以通过defaults里面的dataattribute通过传递一个data集合进来对组件进行初始化....有兴趣可以自己实现，思路和上述类似
        }
    ...
    $.fn.bootstrapTreeTable.methods = {
        // return选中记录的id（return的id由configuration中的idattribute指定）
        // 为了兼容bootstrap-table的写法，统一return数组，这里只return了指定的id
    ...
    $.fn.bootstrapTreeTable.defaults = {
        id : 'menuId',// 选取记录return的值
        code : 'menuId',// 用于设置父子关系
        parentCode : 'parentId',// 用于设置父子关系
        rootCodeValue: null,//设置根节点code值----可指定根节点，默认为null,"",0,"0"
        data : [], // 构造table的data集合
        type : "GET", // 请求data的ajaxtype
        url : null, // 请求data的ajax的url
        ajaxParams : {}, // 请求data的ajax的dataattribute
        expandColumn : null,// 在哪一列上面 view展开button
        expandAll : true, // isno全部展开
        striped : false, // isno各行渐变色
        columns : [],
        toolbar: null,//顶部工具条
        height: 0,
        expanderExpandedClass : 'glyphicon glyphicon-chevron-down',// 展开的button的icon
        expanderCollapsedClass : 'glyphicon glyphicon-chevron-right'// 缩起的button的icon
